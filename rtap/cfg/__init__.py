...
# Define valid tasks and modes, ordered as they appear across the docs and RTAP Platform
...
_YOLO_CLI_COMMAND = [sys.executable, "-m", "rtap.cfg.__init__"]
SOLUTIONS_HELP_MSG = f"""
    Arguments received: {["yolo", *ARGV[1:]]!s}. RTAP 'rtap solutions' usage overview:

        rtap solutions SOLUTION ARGS
    ...
    1. Call object counting solution
        rtap solutions count source="path/to/video.mp4" region="[(20, 400), (1080, 400), (1080, 360), (20, 360)]"

    2. Call heatmap solution
        rtap solutions heatmap colormap=cv2.COLORMAP_PARULA model=yolo26n.pt

    3. Call queue management solution
        rtap solutions queue region="[(20, 400), (1080, 400), (1080, 360), (20, 360)]" model=yolo26n.pt

    4. Call workout monitoring solution for push-ups
        rtap solutions workout model=yolo26n-pose.pt kpts=[6, 8, 10]

    5. Generate analytical graphs
        rtap solutions analytics analytics_type="pie"

    6. Track objects within specific zones
        rtap solutions trackzone source="path/to/video.mp4" region="[(150, 150), (1130, 150), (1130, 570), (150, 570)]"

    7. Count objects inside specific regions
        rtap solutions region source="path/to/video.mp4" region="[(20, 400), (1080, 400), (1080, 360), (20, 360)]"

    8. Run security alarm monitoring (email alerts require Python API)
        rtap solutions security source="path/to/video.mp4"

    9. Monitor parking occupancy (create JSON annotations first via Python ParkingPtsSelection)
        rtap solutions parking source="path/to/video.mp4" json_file="bounding_boxes.json"

    10. Streamlit real-time webcam inference GUI
        rtap streamlit-predict
    """
CLI_HELP_MSG = f"""
    Arguments received: {["yolo", *ARGV[1:]]!s}. RTAP 'rtap' commands use the following syntax:
    ...
    1. Train a detection model for 10 epochs with an initial learning_rate of 0.01
        rtap train data=coco8.yaml model=yolo26n.pt epochs=10 lr0=0.01

    2. Predict a YouTube video using a pretrained segmentation model at image size 320:
        rtap predict model=yolo26n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

    3. Validate a pretrained detection model at batch-size 1 and image size 640:
        rtap val model=yolo26n.pt data=coco8.yaml batch=1 imgsz=640
    ...
    5. RTAP solutions usage
        rtap solutions count or any of {list(SOLUTION_MAP.keys())[1:-1]} source="path/to/video.mp4"
...
QUANTIZE_DOCS_URL = "/modes/export/#quantization-options"
...
def check_cfg(cfg: dict, hard: bool = True) -> None:
    """Check configuration argument types and values for the RTAP library.
...
def get_save_dir(args: SimpleNamespace, name: str | None = None) -> Path:
    ...
    if getattr(args, "save_dir", None):
        ...
        from rtap.utils.files import increment_path
...
def handle_yolo_login(args: list[str]) -> None:
    """Log in to RTAP Platform with an API key or remove the saved key."""
    if args[0] == "logout":
        SETTINGS["api_key"] = ""
        LOGGER.info("Logged out ✅. To log in again, use 'rtap login API_KEY'.")
        return

    api_key_url = f"{PLATFORM_URL}/settings?tab=api-keys"
    if len(args) < 2:
        LOGGER.info(f"Get an API key from {api_key_url} and then run 'rtap login API_KEY'.")
        return
...
def handle_yolo_settings(args: list[str]) -> None:
    ...
    Notes:
        ...
        - For more information on handling YOLO settings, visit:
          /quickstart/#rtap-settings
    """
    url = "/quickstart/#rtap-settings"  # help URL
    try:
        if any(args):
            if args[0] == "reset":
        ...
        LOGGER.info(f"💡 Learn more about RTAP Settings at {url}")
...
def handle_yolo_solutions(args: list[str]) -> None:
    ...
    Args:
        args (list[str]): Command-line arguments for configuring and running the RTAP solutions.
    ...
    Notes:
        ...
        - The Streamlit app file is located in the RTAP package directory.
    """
    from rtap.solutions.config import SolutionConfig
    ...
    # Get solution name
    if not args:
        LOGGER.warning("No solution name provided. i.e `rtap solutions count`. Defaulting to 'count'.")
        args = ["count"]
    if args[0] == "help":
    ...
    if solution_name == "inference":
        ...
        LOGGER.info("💡 Loading RTAP live inference app...")
        subprocess.run(
...
def entrypoint(debug: str = "") -> None:
    """RTAP entrypoint function for parsing and executing command-line arguments.

    This function serves as the main entry point for the RTAP CLI, parsing command-line arguments and executing
    ...
    if mode in {"predict", "track"} and "source" not in overrides:
        overrides["source"] = (
            "https://images.pexels.com/photos/boats.jpg" if task == "obb" else DEFAULT_CFG.source or ASSETS
        )
    ...
    # Show help
    LOGGER.info(f"💡 Learn more at /modes/{mode}")
...
def copy_default_cfg() -> None:
    ...
    LOGGER.info(
        ...
        f"Example YOLO command with this new custom cfg:\n    rtap cfg='{new_file}' imgsz=320 batch=8"
    )


if __name__ == "__main__":
    # Example: entrypoint(debug='rtap predict model=yolo26n.pt')
    entrypoint(debug="")
