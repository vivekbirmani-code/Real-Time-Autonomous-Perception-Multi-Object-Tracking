...
from rtap.data.utils import polygons2masks, polygons2masks_overlap
from rtap.utils import LOGGER, IterableSimpleNamespace, colorstr, deprecation_warn
from rtap.utils.checks import check_version
from rtap.utils.instance import Instances
from rtap.utils.metrics import bbox_ioa
from rtap.utils.ops import segment2box, xywh2xyxy, xyxyxyxy2xywhr
from rtap.utils.torch_utils import TORCHVISION_0_10, TORCHVISION_0_11, TORCHVISION_0_13
...
class BaseTransform:
    """Base class for image transformations in the RTAP library.
...
class Mosaic(BaseMixTransform):
    ...
    Examples:
        >>> from rtap.data.augment import Mosaic
...
class MixUp(BaseMixTransform):
    ...
    Examples:
        >>> from rtap.data.augment import MixUp
        >>> dataset = YourDataset(...)  # Your image dataset
    ...
    def __init__(self, dataset, pre_transform=None, p: float = 0.0) -> None:
        ...
        and labels. This implementation is designed for use with the RTAP framework.
...
class CutMix(BaseMixTransform):
    ...
    Examples:
        >>> from rtap.data.augment import CutMix
        >>> dataset = YourDataset(...)  # Your image dataset
...
class RandomHSV(BaseTransform):
    ...
    Examples:
        >>> import numpy as np
        >>> from rtap.data.augment import RandomHSV
    ...
    def apply_image(self, labels, params: dict[str, Any] | None = None):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            # lut_hue = ((x * (r[0] + 1)) % 180).astype(dtype)   # original hue implementation from rtap<=8.3.78
...
class CopyPaste(BaseMixTransform):
    ...
    Examples:
        >>> from rtap.data.augment import CopyPaste
        >>> dataset = YourDataset(...)  # Your image dataset
...
def v8_transforms(dataset, imgsz: int, hyp: IterableSimpleNamespace):
    ...
    Examples:
        >>> from rtap.cfg import DEFAULT_CFG
        >>> from rtap.data.dataset import YOLODataset
        >>> from rtap.utils import IterableSimpleNamespace
...
class ToTensor:
    ...
    def __init__(self, half: bool = False):
        ...
        RTAP framework. It converts numpy arrays or PIL Images to PyTorch tensors, with an option for
