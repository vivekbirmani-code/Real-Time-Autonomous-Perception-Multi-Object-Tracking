...
from rtap.utils import ASSETS_URL, DATASETS_DIR, LOGGER, NUM_THREADS, TQDM, YAML, clean_url
from rtap.utils.checks import check_file
from rtap.utils.downloads import download, zip_directory
...
def convert_coco(
    ...
    Examples:
        >>> from rtap.data.converter import convert_coco
...
def convert_segment_masks_to_yolo_seg(masks_dir: str, output_dir: str, classes: int):
    ...
    Examples:
        >>> from rtap.data.converter import convert_segment_masks_to_yolo_seg
...
def convert_dota_to_yolo_obb(dota_root_path: str):
    ...
    Examples:
        >>> from rtap.data.converter import convert_dota_to_yolo_obb
...
def yolo_bbox2segment(im_dir: str | Path, save_dir: str | Path | None = None, sam_model: str = "sam_b.pt", device=None):
    ...
    from rtap.data import YOLODataset
    from rtap.utils.ops import xywh2xyxy
...
def create_synthetic_coco_dataset():
    ...
    Examples:
        >>> from rtap.data.converter import create_synthetic_coco_dataset
...
def convert_to_multispectral(path: str | Path, n_channels: int = 10, replace: bool = False, zip: bool = False):
    ...
    """
    from rtap.data.utils import IMG_FORMATS
...
def _infer_ndjson_kpt_shape(image_records: list) -> list:
    ...
    if not kpt_lengths or len(set(kpt_lengths)) != 1:
        raise ValueError("Pose dataset missing required 'kpt_shape'. See /datasets/pose/")
...
async def convert_ndjson_to_yolo(ndjson_path: str | Path, output_path: str | Path | None = None) -> Path:
    """Convert NDJSON dataset format to RTAP dataset structure.
