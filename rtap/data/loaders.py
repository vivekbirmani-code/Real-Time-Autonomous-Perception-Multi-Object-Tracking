...
from rtap.data.utils import FORMATS_HELP_MSG, IMG_FORMATS, VID_FORMATS
from rtap.utils import IS_COLAB, IS_KAGGLE, LOGGER, ops
from rtap.utils.checks import check_requirements
from rtap.utils.patches import imread
...
class LoadScreenshots:
    """RTAP screenshot dataloader for capturing and processing screen images.
...
class LoadImagesAndVideos:
    ...
    def __init__(self, path: str | Path | list, batch: int = 1, vid_stride: int = 1, channels: int = 3):
        ...
        for p in sorted(path) if isinstance(path, (list, tuple)) else [path]:
            a = str(Path(p).absolute())  # do not use .resolve() 
            if "*" in a:
...
def autocast_list(source: list[Any]) -> list[Image.Image | np.ndarray]:
    """Convert a list of sources into a list of numpy arrays or PIL images for RTAP prediction."""
    files = []
    for im in source:
        if isinstance(im, (str, Path)):  # filename or uri
            ...
            raise TypeError(
                f"type {type(im).__name__} is not a supported RTAP prediction source type. \n"
                f"See /modes/predict for supported source types."
            )
