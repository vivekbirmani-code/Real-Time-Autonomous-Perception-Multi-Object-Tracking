...
from rtap.utils.checks import check_file, check_font, is_ascii, normalize_platform_uri
from rtap.utils.downloads import download, safe_download
from rtap.utils.ops import segments2boxes

HELP_URL = "See /datasets for dataset formatting guidance."
IMG_FORMATS = {
    "avif",
    "bmp",
    "dng",
...
def check_file_speeds(
    ...
    if avg_ping < threshold_ms and avg_speed > threshold_mb:
        ...
        LOGGER.warning(
            ...
            f"See /guides/model-training-tips/"
        )
...
def convert_ndjson_to_yolo_if_needed(data: str | Path) -> str | Path:
    ...
    data = normalize_platform_uri(data)  # accept Platform web URLs (/.../datasets/...)
    data_str = str(data)
...
def check_cls_dataset(dataset: str | Path, split: str = "") -> dict[str, Any]:
    ...
    if not train_set.is_dir():
        ...
        if image_files := [f for f in data_dir.rglob("*.*") if f.suffix[1:].lower() in IMG_FORMATS]:
            from rtap.data.split import split_classify_dataset
...
def compress_one_image(f: str, f_new: str | None = None, max_dim: int = 1920, quality: int = 50):
    ...
    Examples:
        >>> from pathlib import Path
        >>> from rtap.data.utils import compress_one_image
...
def load_dataset_cache_file(path: Path) -> dict:
    """Load an RTAP *.cache dictionary from path."""
    import gc
...
def save_dataset_cache_file(prefix: str, path: Path, x: dict, version: str):
    """Save an RTAP dataset *.cache dictionary x to path."""
