...
from rtap.cfg import QUANTIZE_DOCS_URL, TASK2CALIBRATIONDATA, TASK2DATA, get_cfg
from rtap.data import build_dataloader, build_yolo_dataset
from rtap.data.dataset import ClassificationDataset
from rtap.data.utils import check_cls_dataset, check_det_dataset
from rtap.nn.autobackend import AutoBackend, check_class_names, default_class_names
...
from rtap.nn.tasks import ClassificationModel, DepthModel, DetectionModel, SegmentationModel, WorldModel
...
from rtap.utils.files import file_size
from rtap.utils.metrics import batch_probiou
from rtap.utils.nms import TorchNMS
from rtap.utils.ops import Profile
from rtap.utils.patches import arange_patch
...
def export_formats():
    """Return a dictionary of RTAP export formats."""
...
class Exporter:
    ...
    def __call__(self, model=None) -> str:
        ...
        if fmt in {"tflite", "tfjs"}:  # deprecated formats, replaced by the unified Google LiteRT export
            LOGGER.warning(
                ...
                f"format. Exporting format='litert' instead. See /integrations/litert/"
            )
                ...
                try:
                    ...
                    if (
                        ...
                        LOGGER.warning(
                            ...
                            "For a fix, see /guides/nvidia-jetson/#why-does-my-tensorrt-int8-export-disable-end2end-on-jetpack-6"
                            ""
                        )
                except ImportError:
                    pass
        ...
        if fmt == "ascend":
            ...
            if not str(self.args.name).startswith("Ascend"):
                raise ValueError(
                    ...
                    f"'Ascend310P3' or 'Ascend310B4'. See /integrations/ascend/"
                )
        ...
        if isinstance(model, WorldModel):
            LOGGER.warning(
                ...
                "See /models/yolo-world for details."
            )
            model.clip_model = None  # openvino int8 export error: 
        ...
        if SETTINGS.get("openvino_msg"):
            if is_intel():
                LOGGER.info(
                    ...
                    " Learn more at /integrations/openvino/"
                )
        ...
        if fmt == "imx":
            from rtap.utils.export.imx import FXModel

            model = FXModel(model, self.imgsz)
        if fmt == "edgetpu":
            from rtap.utils.export.tensorflow import tf_wrapper

            model = tf_wrapper(model)
        if fmt == "executorch":
            from rtap.utils.export.executorch import executorch_wrapper
        ...
        description = f"RTAP {self.pretty_name} model {f'trained on {data}' if data else ''}"
        self.metadata = {
    ...
    def get_int8_calibration_dataloader(self, prefix=""):
        ...
        if self.model.task == "classify":
            import torchvision.transforms as T  # scope for faster 'import rtap'
    ...
    def export_torchscript(self, prefix=colorstr("TorchScript:")):  # noqa: B008
        ...
        from rtap.utils.export.torchscript import torch2torchscript
    ...
    def export_onnx(self, prefix=colorstr("ONNX:")):  # noqa: B008
        ...
        from rtap.utils.export.engine import best_onnx_opset, torch2onnx
        ...
        if (
            ...
            from rtap.utils.export.engine import _NormalizeCoords
    ...
    def export_openvino(self, prefix=colorstr("OpenVINO:")):  # noqa: B008
        ...
        from rtap.utils.export.openvino import torch2openvino
    ...
    def export_paddle(self, prefix=colorstr("PaddlePaddle:")):  # noqa: B008
        ...
        from rtap.utils.export.paddle import torch2paddle
    ...
    def export_litert(self, prefix=colorstr("LiteRT:")):  # noqa: B008
        ...
        from rtap.utils.export.litert import torch2litert
    ...
    def export_mnn(self, prefix=colorstr("MNN:")):  # noqa: B008
        ...
        from rtap.utils.export.mnn import onnx2mnn
    ...
    def export_ncnn(self, prefix=colorstr("NCNN:")):  # noqa: B008
        ...
        from rtap.utils.export.ncnn import torch2ncnn
    ...
    def export_coreml(self, prefix=colorstr("CoreML:")):  # noqa: B008
        ...
        from rtap.utils.export.coreml import IOSDetectModel, pipeline_coreml, torch2coreml
    ...
    def export_engine(self, prefix=colorstr("TensorRT:")):  # noqa: B008
        ...
        f_onnx = self.export_onnx()  # run before TRT import 
        from rtap.utils.export.engine import onnx2engine
    ...
    def export_saved_model(self, prefix=colorstr("TensorFlow SavedModel:")):  # noqa: B008
        ...
        from rtap.utils.export.tensorflow import onnx2saved_model
    ...
    def export_pb(self, keras_model, prefix=colorstr("TensorFlow GraphDef:")):  # noqa: B008
        ...
        from rtap.utils.export.tensorflow import keras2pb
    ...
    def export_axelera(self, prefix=colorstr("Axelera:")):  # noqa: B008
        ...
        from rtap.utils.export.axelera import torch2axelera
    ...
    def export_executorch(self, prefix=colorstr("ExecuTorch:")):  # noqa: B008
        ...
        from rtap.utils.export.executorch import torch2executorch
    ...
    def export_edgetpu(self, tflite_model="", prefix=colorstr("Edge TPU:")):  # noqa: B008
        ...
        from rtap.utils.export.tensorflow import tflite2edgetpu
    ...
    def export_rknn(self, prefix=colorstr("RKNN:")):  # noqa: B008
        ...
        from rtap.utils.export.rknn import onnx2rknn
    ...
    def export_ascend(self, prefix=colorstr("Ascend:")):  # noqa: B008
        ...
        from rtap.utils.export.ascend import _check_atc, onnx2ascend
    ...
    def export_imx(self, prefix=colorstr("IMX:")):  # noqa: B008
        ...
        from rtap.utils.export.imx import torch2imx
    ...
    def export_deepx(self, prefix=colorstr("DEEPX:")):  # noqa: B008
        ...
        from rtap.utils.export.deepx import onnx2deepx
    ...
    def export_qnn(self, prefix=colorstr("Qualcomm QNN:")):  # noqa: B008
        ...
        from rtap.utils.export.qnn import onnx2qnn
    ...
    def export_hailo(self, prefix=colorstr("Hailo:")):  # noqa: B008
        ...
        LOGGER.warning(
            ...
            "See /integrations/hailo/#export-a-hailo-hef-model"
        )
