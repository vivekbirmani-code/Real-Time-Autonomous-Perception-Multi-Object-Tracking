...
from rtap.cfg import TASK2DATA, _handle_deprecation, get_cfg, get_save_dir
from rtap.engine.results import Results
from rtap.nn.tasks import BaseModel, guess_model_task, load_checkpoint, yaml_model_load
...
class Model(torch.nn.Module):
    ...
    Attributes:
        ...
        metrics (rtap.utils.metrics.DetMetrics): The latest training/validation metrics.
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        Returns:
            (Iterator[rtap.engine.results.Results | torch.Tensor] | list[rtap.engine.results.Results] |
            list[torch.Tensor]): Prediction results or embeddings, streamed when `stream=True`.

        Examples:
            >>> model = YOLO("yolo26n.pt")
            >>> results = model("https://images.pexels.com/photos/bus.jpg")
            >>> for r in results:
    ...
    def save(self, filename: str | Path = "saved_model.pt") -> None:
        ...
        date, RTAP version, license information, and a link to the documentation.
    ...
    def embed(
        ...
        **kwargs: Any,
        ...
        Raises:
            TypeError: If the model is not an RTAP PyTorch model. Exported formats and third-party modules expose
                no intermediate layers to embed.

        Examples:
            >>> model = YOLO("yolo26n.pt")
            >>> image = "https://images.pexels.com/photos/bus.jpg"
        ...
        if not isinstance(model, BaseModel):  # e.g. a super-gradients YOLO-NAS module, which has no layer list
            raise TypeError(f"model='{type(model).__name__}' is not an RTAP model and cannot be embedded.")
    ...
    def predict(
        ...
        **kwargs: Any,
        ...
        """
        if source is None:
            source = "https://images.pexels.com/photos/boats.jpg" if self.task == "obb" else ASSETS
            LOGGER.warning(f"'source' is missing. Using 'source={source}'.")

        is_cli = (ARGV[0].endswith("yolo") or ARGV[0].endswith("rtap")) and any(
    ...
    def track(
        ...
        **kwargs: Any,
        ...
        Returns:
            (list[rtap.engine.results.Results]): A list of tracking results, each a Results object.
        ...
        """
        from rtap.trackers import register_tracker
    ...
    def val(
        ...
        **kwargs: Any,
        ...
        Args:
            validator (rtap.engine.validator.BaseValidator, optional): An instance of a custom validator class
                for validating the model.
            **kwargs (Any): Arbitrary keyword arguments for customizing the validation process.

        Returns:
            (rtap.utils.metrics.DetMetrics): Validation metrics obtained from the validation process. The
    ...
    def calibrate(self, data=None, **kwargs: Any):
        ...
        from rtap.models.yolo.depth.calibrate import _depth_head, fit_calibration_selective
    ...
    def benchmark(self, data=None, format="", verbose=False, **kwargs: Any):
        ...
        uses the 'benchmark' function from the rtap.utils.benchmarks module. The benchmarking is configured using
        ...
        """
        self._check_is_pytorch_model()
        from rtap.utils.benchmarks import benchmark
    ...
    def export(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            LOGGER.info(f"Export to {format_name} in the cloud with RTAP Platform: {PLATFORM_URL}")
            raise
    ...
    def train(
        ...
        **kwargs: Any,
        ...
        Returns:
            (rtap.utils.metrics.DetMetrics | dict | None): Training metrics if available and training is
        ...
        if isinstance(args.get("data"), (list, tuple)):  # fine-tune a single base model across multiple datasets
            from rtap.engine.trainer import MultiTrainer
    ...
    def tune(
        ...
        **kwargs: Any,
        ...
        is enabled, it leverages the 'run_ray_tune' function from the rtap.utils.tuner module. Otherwise, it uses
        ...
        self._check_is_pytorch_model()
        if use_ray:
            from rtap.utils.tuner import run_ray_tune
    ...
    def names(self) -> dict[int, str]:
        ...
        using the 'check_class_names' function from the rtap.nn.autobackend module. If the predictor is not
    ...
    def clear_callback(self, event: str) -> None:
        ...
        Args:
            event (str): The name of the event for which to clear the callbacks. This should be a valid event name
                recognized by the RTAP callback system.
        ...
        Notes:
            - This method affects both custom callbacks added by the user and default callbacks
              provided by the RTAP framework.
