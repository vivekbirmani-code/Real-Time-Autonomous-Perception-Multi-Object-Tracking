...
from rtap.engine.model import Model
from rtap.nn.tasks import RTDETRDetectionModel
from rtap.utils.torch_utils import TORCH_1_11
...
class RTDETR(Model):
    ...
    Methods:
        task_map: Return a task map for RT-DETR, associating tasks with corresponding RTAP classes.
    ...
    def task_map(self) -> dict:
        """Return a task map for RT-DETR, associating tasks with corresponding RTAP classes.

        Returns:
            (dict): A dictionary mapping task names to RTAP task classes for the RT-DETR model.
        """
        return {
            "detect": {
