...
from rtap.utils.checks import check_suffix
from rtap.utils.downloads import is_url
...
def check_class_names(names: list | dict) -> dict[int, str]:
    ...
    if isinstance(names, dict):
        ...
        if isinstance(names[0], str) and names[0].startswith("n0"):  # imagenet class codes, i.e. 'n01440764'
            from rtap.utils import ROOT, YAML
...
def default_class_names(data: str | Path | None = None) -> dict[int, str]:
    ...
    if data:
        try:
            ...
            from rtap.utils.checks import check_yaml
...
class AutoBackend(nn.Module):
    """Handle dynamic backend selection for running inference using RTAP models.
    ...
    def __init__(
        ...
        if format not in self._BACKEND_MAP:
            ...
            raise TypeError(
                ...
                f"RTAP supports: {export_formats()['Format']}\n"
                f"See /modes/predict for help."
            )
        if format == "pt":
