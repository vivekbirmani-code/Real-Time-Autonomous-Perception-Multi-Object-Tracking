...
def read_tflite_metadata(file: str | Path) -> dict | None:
    """Read RTAP metadata embedded in a ``.tflite`` file.

    RTAP appends metadata to the end of ``.tflite`` flatbuffers as a zip entry (``metadata.json`` for
...
class BaseBackend(ABC):
    ...
    Attributes:
        ...
        base_model (bool): Whether the loaded model is an RTAP `BaseModel`, and so implements the `augment` and
