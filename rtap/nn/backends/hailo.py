...
class HailoBackend(BaseBackend):
    """HailoRT inference backend for RTAP Hailo HEF models."""

    def load_model(self, weight: str | Path) -> None:
        """Load a Hailo export directory and its RTAP metadata."""
        try:
            ...
            raise ImportError(
                ...
                "See /integrations/hailo/#run-hailo-inference"
            ) from e
        ...
        if self.task in {"segment", "pose", "obb"}:
            from rtap.nn.modules import DFL
    ...
    def _decode_boxes(self, reg_maps: list[torch.Tensor], angle: torch.Tensor | None = None) -> torch.Tensor:
        ...
        from rtap.utils.tal import dist2bbox, dist2rbox, make_anchors
    ...
    def _decode_raw(self, outputs: list[np.ndarray]) -> np.ndarray:
        ...
        from rtap.utils.tal import dist2bbox, make_anchors
