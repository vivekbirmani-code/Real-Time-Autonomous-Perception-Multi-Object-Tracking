...
from rtap.nn.autobackend import check_class_names
from rtap.nn.modules import (
    AIFI,
    C1,
    C2,
    C2PSA,
...
from rtap.utils.checks import REMOTE_FILE_PREFIXES, check_file, check_requirements, check_suffix, check_yaml
from rtap.utils.loss import (
    DepthLoss26,
    E2ELoss,
    PoseLoss26,
...
from rtap.utils.ops import make_divisible
...
class BaseModel(torch.nn.Module):
    """Base class for all YOLO models in the RTAP family.
    ...
    def _profile_one_layer(self, m, x, dt):
        ...
        try:
            ...
            thop = None  # conda support without 'thop' installed
...
class SegmentationModel(DetectionModel):
    ...
    def __init__(self, cfg="yolo26n-seg.yaml", ch=3, nc=None, verbose=True):
        """Initialize RTAP segmentation model with given config and parameters.
...
class ClassificationModel(BaseModel):
    ...
    def _from_yaml(self, cfg, ch, nc, verbose):
        """Set RTAP model configurations and define the model architecture.
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def init_criterion(self):
        ...
        from rtap.models.utils.loss import RTDETRDetectionLoss
...
class WorldModel(DetectionModel):
    ...
    def get_text_pe(self, text, batch=80, cache_clip_model=True):
        ...
        """
        from rtap.nn.text_model import build_text_model
...
class YOLOEModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        """
        if not hasattr(self, "criterion"):
            from rtap.utils.loss import TVPDetectLoss
...
class YOLOESegModel(YOLOEModel, SegmentationModel):
    ...
    def loss(self, batch, preds=None):
        ...
        """
        if not hasattr(self, "criterion"):
            from rtap.utils.loss import TVPSegmentLoss
...
class _SafeLoad:
    ...
    Enabled per-process by the `RTAP_SAFE_LOAD` env flag, or per-call by `torch_safe_load(..., safe_only=True)`.
    ...
    @classmethod
    def _build(cls):
        """Auto-discover `nn.Module` subclasses across `torch.nn` and the rtap model families, registered under
        ...
        import rtap.nn.modules as ul_nn
        from rtap.nn import tasks as ul_tasks  # noqa: PLW0406
        ...
        _scan(ul_nn)  # rtap block/conv/head/transformer
        _scan(ul_tasks)  # rtap task models

        # Non-nn.Module data globals in official checkpoints, incl. the pre-8.0.44 `rtap.yolo.utils` path.
        allow.append(IterableSimpleNamespace)
        allow.append((IterableSimpleNamespace, "rtap.yolo.utils.IterableSimpleNamespace"))
        ...
        from rtap.utils.loss import E2EDetectLoss
        ...
        allow += [
            (nn.Identity, "rtap.nn.modules.block.Silence"),  # YOLOv9e
            (DetectionModel, "rtap.nn.tasks.YOLOv10DetectionModel"),  # YOLOv10
            (E2EDetectLoss, "rtap.utils.loss.v10DetectLoss"),  # YOLOv10
...
def torch_safe_load(weight, safe_only=None):
    ...
    Args:
        ...
        safe_only (bool, optional): Load with `torch.load(weights_only=True)`, reconstructing only the known
            RTAP/torch model classes on the allow-list. Defaults to the `RTAP_SAFE_LOAD` environment
    ...
    Examples:
        >>> from rtap.nn.tasks import torch_safe_load
        >>> ckpt, file = torch_safe_load("path/to/best.pt", safe_only=True)
    """
    from rtap.utils.downloads import GITHUB_ASSETS_NAMES, attempt_download_asset
    ...
    def _load():
        with temporary_modules(
            modules={
                "rtap.yolo.utils": "rtap.utils",
                "rtap.yolo.v8": "rtap.models.yolo",
                "rtap.yolo.data": "rtap.data",
            },
            attributes={
                "rtap.nn.modules.block.Silence": "torch.nn.Identity",  # YOLOv9e
                "rtap.nn.tasks.YOLOv10DetectionModel": "rtap.nn.tasks.DetectionModel",  # YOLOv10
                "rtap.utils.loss.v10DetectLoss": "rtap.utils.loss.E2EDetectLoss",  # YOLOv10
    ...
    torchscript_error = emojis(
        f"ERROR ❌️ {weight} is a TorchScript archive, not an RTAP PyTorch checkpoint.\n"
    ...
    try:
        ...
        if safe_only and isinstance(e, pickle.UnpicklingError):
            ...
            raise TypeError(
                emojis(
                    f"ERROR ❌️ {weight} references types outside the supported RTAP checkpoint format. "
                    f"Use an official RTAP model, i.e. 'yolo predict model=yolo26n.pt'"
                )
            ) from e
        ...
        if str(weight) != name or name not in GITHUB_ASSETS_NAMES:
            raise TypeError(
                emojis(
                    ...
                    f"run a command with an official RTAP model, i.e. 'yolo predict model=yolo26n.pt'"
                )
            ) from e
        ...
        if e.name in {"models", "models.yolo", "models.common", "models.experimental"}:
            raise TypeError(
                emojis(
                    f"ERROR ❌️ {weight} appears to be an RTAPv5 model originally trained "
                    f"with https://github.com/rtap/yolov5. This model is NOT forwards compatible with "
                    f"YOLOv8 at "
                    f"\nRecommend fixes are to train a new model using the latest 'rtap' package or to "
                    f"run a command with an official RTAP model, i.e. 'yolo predict model=yolo26n.pt'"
                )
            ) from e
        ...
        elif e.name and e.name.startswith("rtap."):
            raise ModuleNotFoundError(
                emojis(
                    f"ERROR ❌️ {weight} requires missing RTAP module '{e.name}'. "
                    "Train a new model using the latest 'rtap' package or run a command with an official "
                    "RTAP model, i.e. 'yolo predict model=yolo26n.pt'"
                )
            ) from e
        if safe_only:
            ...
            raise
        LOGGER.warning(
            f"{weight} appears to require '{e.name}', which is not in RTAP requirements."
...
def yaml_model_load(path):
    ...
    if path.stem in (f"yolov{d}{x}6" for x in "nsmlx" for d in (5, 8)):
        ...
        LOGGER.warning(f"RTAP P6 models now use -p6 suffix. Renaming {path.stem} to {new_stem}.")
