...
from rtap import __version__
from rtap.utils.git import GitRepo
from rtap.utils.patches import imread, imshow, imwrite, torch_save  # for patches
from rtap.utils.tqdm import TQDM  # noqa
...
ASSETS_URL = os.getenv("RTAP_ASSETS_URL", "").rstrip("/")  # optional remote demo assets base URL
# Configurable Platform URL for debugging (e.g. RTAP_PLATFORM_URL=http://localhost:3000)
PLATFORM_URL = os.getenv("RTAP_PLATFORM_URL", "").rstrip("/")
...
SAFE_LOAD = env_bool("RTAP_SAFE_LOAD")  # opt-in weights_only model loading
LOGGING_NAME = "rtap"
...
HELP_MSG = """
    Examples for running RTAP:

    1. Install the rtap package:

        pip install rtap-mot
    ...
    2. Use the Python SDK:
        ...
        results = model("https://images.pexels.com/photos/bus.jpg")  # predict on an image
        success = model.export(format="onnx")  # export the model to ONNX format

    3. Use the command line interface (CLI):

        RTAP 'rtap' CLI commands use the following syntax:
            ...
            Where   TASK (optional) is one of [detect, segment, semantic, classify, pose, obb, depth]
                    ...
                    ARGS (optional) are any number of custom "arg=value" pairs like "imgsz=320" that override defaults.
                        See all ARGS at /usage/cfg or with "rtap cfg"

        - Train a detection model for 10 epochs with an initial learning_rate of 0.01
            rtap detect train data=coco8.yaml model=yolo26n.pt epochs=10 lr0=0.01

        - Predict a YouTube video using a pretrained segmentation model at image size 320:
            rtap segment predict model=yolo26n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320

        - Val a pretrained detection model at batch-size 1 and image size 640:
            rtap detect val model=yolo26n.pt data=coco8.yaml batch=1 imgsz=640

        - Export a YOLO26n classification model to ONNX format at image size 224 by 128 (no TASK required)
            rtap export model=yolo26n-cls.pt format=onnx imgsz=224,128
...
class DataExportMixin:
    ...
    def to_df(self, normalize=False, decimals=5):
        ...
        """
        import polars as pl  # scope for faster 'import rtap'
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date RTAP
            'default.yaml' file.\nPlease update your code with 'pip install -U rtap-mot' and if necessary replace
...
def plt_settings(rcparams=None, backend="Agg"):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            import matplotlib.pyplot as plt  # scope for faster 'import rtap'
            ...
            if "font.sans-serif" not in rcparams and not wrapper._fonts_registered:
                ...
                # Register any fonts in RTAP config dir (e.g. Arial.Unicode.ttf) with matplotlib
...
def set_logging(name="LOGGING_NAME", verbose=True):
    ...
    This function configures logging for the RTAP library, setting the appropriate logging level and formatter
    ...
    Examples:
        >>> set_logging(name="rtap", verbose=True)
        >>> logger = logging.getLogger("rtap")
    ...
    stream_handler.set_name("rtap.utils.set_logging")
...
class ThreadingLocked:
    ...
    Examples:
        >>> from rtap.utils import ThreadingLocked
        >>> @ThreadingLocked()
        ... def my_function():
        ...    # Your code here
    """
...
def get_user_config_dir(sub_dir="RTAP"):
...
IS_UBUNTU = is_ubuntu()
GIT = GitRepo()
USER_CONFIG_DIR = get_user_config_dir()  # RTAP settings dir
...
class TryExcept(contextlib.ContextDecorator):
    """RTAP TryExcept class for handling exceptions gracefully.
...
def set_sentry():
    ...
    Only used if sentry_sdk package is installed and sync=True in settings. Run 'rtap settings' to see and update
    settings.
...
class SettingsManager(JSONDict):
    """SettingsManager class for managing and persisting RTAP settings.
    ...
    def __init__(self, file=SETTINGS_FILE, version="0.0.7"):
        ...
        from rtap.utils.torch_utils import torch_distributed_zero_first
        ...
        self.help_msg = (
            f"\nView RTAP Settings with 'rtap settings' or at '{self.file}'"
            "\nUpdate Settings with 'rtap settings key=value', i.e. 'rtap settings runs_dir=path/to/dir'. "
            "For help see /quickstart/#rtap-settings."
        )
        ...
        with torch_distributed_zero_first(LOCAL_RANK):
            ...
            if not self.file.exists() or not self:  # Check if file doesn't exist or is empty
                LOGGER.info(f"Creating new RTAP Settings v{version} file ✅ {self.help_msg}")
                self.reset()
    ...
    def _validate_settings(self):
        ...
        if not (correct_keys and correct_types and correct_version):
            LOGGER.warning(
                "RTAP settings updated to the latest schema. Existing values were preserved where possible. "
                f"{self.help_msg}"
            )
        ...
        if self.get("datasets_dir") == self.get("runs_dir"):
            LOGGER.warning(
                f"RTAP setting 'datasets_dir: {self.get('datasets_dir')}' "
    ...
    def update(self, *args, **kwargs):
        ...
        for k, v in kwargs.items():
            if k not in self.defaults:
                raise KeyError(f"No RTAP setting '{k}'. {self.help_msg}")
            t = type(self.defaults[k])
            if not isinstance(v, t):
                raise TypeError(
                    f"RTAP setting '{k}' must be '{t.__name__}' type, not '{type(v).__name__}'. {self.help_msg}"
                )
        super().update(*args, **kwargs)
...
def vscode_msg(ext="rtap.rtap-snippets") -> str:
    """Display a message to install RTAP-Snippets for VS Code if not already installed."""
    path = (USER_CONFIG_DIR.parents[2] if WINDOWS else USER_CONFIG_DIR.parents[1]) / ".vscode/extensions"
    obs_file = path / ".obsolete"  # file tracks uninstalled extensions, while source directory remains
    installed = any(path.glob(f"{ext}*")) and ext not in (obs_file.read_text("utf-8") if obs_file.exists() else "")
    url = "/integrations/vscode"
    return "" if installed else f"{colorstr('VS Code:')} view RTAP VS Code Extension ⚡ at {url}"


# Run below code on utils init ------------------------------------------------------------------------------------

# Check first-install steps
PREFIX = colorstr("RTAP: ")
