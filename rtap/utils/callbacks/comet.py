...
from rtap.utils import LOGGER, RANK, SETTINGS, TESTS_RUNNING, env_bool, ops
from rtap.utils.metrics import ClassifyMetrics, DetMetrics, OBBMetrics, PoseMetrics, SegmentMetrics
...
try:
    ...
    # Names of plots created by RTAP that are logged to Comet
...
def _get_comet_model_name() -> str:
    """Return the Comet model name from environment variable or default to 'RTAP'."""
    return os.getenv("COMET_MODEL_NAME", "RTAP")
...
def _resume_or_create_experiment(args: SimpleNamespace) -> None:
    ...
    try:
        ...
        experiment.log_other("Created from", "rtap")
...
def _fetch_trainer_metadata(trainer) -> dict:
    ...
    Args:
        trainer (rtap.engine.trainer.BaseTrainer): The YOLO trainer object containing training state and config.
...
def _log_plots(experiment, trainer) -> None:
    ...
    Args:
        ...
        trainer (rtap.engine.trainer.BaseTrainer): The trainer object containing validation metrics and save
            directory information.

    Examples:
        >>> from rtap.utils.callbacks.comet import _log_plots
...
def on_fit_epoch_end(trainer) -> None:
    ...
    if curr_epoch == 1:
        from rtap.utils.torch_utils import model_info_for_loggers
