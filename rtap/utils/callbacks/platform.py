...
def _upload_model(model_path, project, name, progress=False, retry=1, model_id=None, run_id=None):
    ...
    from rtap.utils.uploads import safe_upload
...
def _get_environment_info():
    """Collect comprehensive environment info using existing rtap utilities."""
    import shutil
...
def on_pretrain_routine_start(trainer):
    ...
    if TESTS_RUNNING or not trainer.args.project:
        return
    _api_key = os.getenv("RTAP_API_KEY") or SETTINGS.get("api_key")
    if not _api_key:
        return
    ...
    from rtap.utils.logger import ConsoleLogger
...
def on_fit_epoch_end(trainer):
    ...
    try:
        if not ctx["system_logger"]:
            from rtap.utils.logger import SystemLogger
