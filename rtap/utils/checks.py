...
def normalize_platform_uri(uri):
    """Rewrite an RTAP Platform web URL to its ul:// URI so it can be loaded directly as data or model.

    Args:
        uri (str | Path): Resource identifier, e.g. an RTAP Platform web URL ending in "/user/datasets/slug".
...
def resolve_platform_uri(uri, hard=True):
    """Resolve ul:// URIs to signed URLs by authenticating with RTAP Platform.
    ...
    from rtap.utils import SETTINGS

    path = str(uri)[5:]
    parts = path.split("/")
    api_key = os.getenv("RTAP_API_KEY") or SETTINGS.get("api_key")
    if not api_key:
        raise ValueError(f"RTAP_API_KEY required for '{uri}'. Get a key at {PLATFORM_URL}/settings")
    ...
    if response.status_code == 401:
        raise ValueError(f"Invalid RTAP_API_KEY for '{uri}'.{detail}")
...
def parse_requirements(file_path=ROOT.parent / "requirements.txt", package=""):
    ...
    Examples:
        >>> from rtap.utils.checks import parse_requirements
        >>> parse_requirements(package="rtap")
    """
    if package:
...
def check_version(
    ...
    if not current:  # if current is '' or None
        ...
        return True
    elif not current[0].isdigit():  # current is package name rather than version string, i.e. current='rtap'
        try:
...
def check_latest_pypi_version(package_name="rtap"):
...
def check_pip_update_available():
    """Check if a new version of the rtap package is available on PyPI.
    ...
    if ONLINE and IS_PIP_PACKAGE:
        try:
            ...
            if check_version(__version__, f"<{latest}"):  # check if current version is < latest version
                LOGGER.info(
                    f"New https://pypi.org/project/rtap/{latest} available 😃 "
                    f"Update with 'pip install -U rtap-mot'"
                )
                return True
        except Exception:
            pass
...
def check_font(font="Arial.ttf"):
    ...
    """
    from matplotlib import font_manager  # scope for faster 'import rtap'
...
def check_apt_requirements(requirements):
    ...
    if missing_packages:
        LOGGER.info(
            f"{prefix} RTAP requirement{'s' * (len(missing_packages) > 1)} {missing_packages} not found, attempting AutoUpdate..."
        )
...
def check_requirements(requirements=ROOT.parent / "requirements.txt", exclude=(), install=True, cmds="", constrain=()):
    """Check if installed dependencies meet RTAP models requirements and attempt to auto-update if needed.
    ...
    Examples:
        ...
        >>> check_requirements("rtap>=8.3.200", cmds="--index-url https://download.pytorch.org/whl/cpu")

        Check multiple packages
        >>> check_requirements(["numpy", "rtap"])
    ...
    if env_bool("RTAP_SKIP_REQUIREMENTS_CHECKS"):
        LOGGER.info(f"{prefix} RTAP_SKIP_REQUIREMENTS_CHECKS detected, skipping requirements check.")
        return True
    ...
    if pkgs:
        ...
        if install and AUTOINSTALL:  # check environment variable
            ...
            LOGGER.info(f"{prefix} RTAP requirement{'s' * (n > 1)} {pkgs} not found, attempting AutoUpdate...")
            try:
                t = time.time()
...
def check_rtapv5u_filename(file: str, verbose: bool = True) -> str:
        ...
        if "u.yaml" in file:
            ...
            if file != original_file and verbose:
                LOGGER.info(
                    ...
                    f"trained with  and feature improved performance vs "
                    f"standard YOLOv5 models trained with https://github.com/rtap/yolov5."
                )
    return file
...
def check_file(file, suffix="", download=True, download_dir=".", hard=True):
    ...
    file = check_rtapv5u_filename(file)  # yolov5n -> yolov5nu
    if (
        not file
        ...
        return file
    elif download and file.lower().startswith("ul://"):  # RTAP Platform URI
        ...
        if uri_path.is_absolute() or ".." in uri_path.parts:
            raise ValueError(f"Unsafe RTAP Platform URI path: {file}")
...
def check_rtap(verbose=True, device=""):
    ...
    from rtap.utils.torch_utils import select_device
...
def collect_system_info():
    ...
    from rtap.utils import ENVIRONMENT  # scope to avoid circular import
    from rtap.utils.torch_utils import get_cpu_info, get_gpu_info
    ...
    package_info = {}
    for r in parse_requirements(package=get_distribution_name("rtap")):
        try:
...
def check_amp(model):
    ...
    Examples:
        ...
        >>> from rtap.utils.checks import check_amp
        >>> model = YOLO("yolo26n.pt").model.cuda()
        >>> check_amp(model)
    """
    from rtap.utils.torch_utils import autocast
    ...
    try:
        ...
        LOGGER.warning(
            ...
            f"Unable to load YOLO26n for AMP checks due to possible RTAP package modifications. {warning_msg}"
        )
...
def check_multiple_install():
    """Check if there are multiple RTAP installations."""
    import sys

    try:
        result = subprocess.run(
            [sys.executable, "-m", "pip", "show", "rtap"], capture_output=True, text=True, check=False
        )
        install_msg = (
            f"Install your local copy in editable mode with 'pip install -e {ROOT.parent}' to avoid "
            "issues. See /quickstart/"
        )
        if result.returncode != 0:
            if "not found" in result.stderr.lower():  # Package not pip-installed but locally imported
                LOGGER.warning(f"RTAP not found via pip but importing from: {ROOT}. {install_msg}")
            return
        yolo_path = (
            Path(re.findall(r"location:\s+(.+)", result.stdout, flags=re.IGNORECASE)[-1]) / "rtap"
        ).resolve()
        if not yolo_path.samefile(ROOT.resolve()):
            LOGGER.warning(
                f"Multiple RTAP installations detected. The `yolo` command uses: {yolo_path}, "
...
def is_intel():
    ...
    """
    from rtap.utils.torch_utils import get_cpu_info
