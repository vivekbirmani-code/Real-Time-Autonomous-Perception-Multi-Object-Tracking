...
def generate_ddp_file(trainer: BaseTrainer) -> str:
    ...
    Args:
        trainer (rtap.engine.trainer.BaseTrainer): The trainer containing training configuration and arguments.
    ...
    content = f"""
# RTAP Multi-GPU training temp file (should be automatically deleted after use)
from pathlib import Path, PosixPath  # For model arguments stored as Path instead of str
overrides = {overrides}

if __name__ == "__main__":
    from {module} import {name}
    from rtap.utils import DEFAULT_CFG_DICT
...
def generate_ddp_command(trainer: BaseTrainer) -> tuple[list[str], str]:
    ...
    Args:
        trainer (rtap.engine.trainer.BaseTrainer): The trainer containing configuration for distributed training.
...
def ddp_cleanup(trainer: BaseTrainer, file: str) -> None:
    ...
    Args:
        trainer (rtap.engine.trainer.BaseTrainer): The trainer used for distributed training.
