...
from rtap.utils import ASSETS_URL, LOGGER, TQDM, checks, clean_url, emojis, is_online, url2file

# Define optional remote weight release repo (set RTAP_WEIGHTS_REPO=owner/repo to enable downloads)
GITHUB_ASSETS_REPO = os.getenv("RTAP_WEIGHTS_REPO", "")
...
def delete_dsstore(path: str | Path, files_to_delete: tuple[str, ...] = (".DS_Store", "__MACOSX")) -> None:
    ...
    Examples:
        >>> from rtap.utils.downloads import delete_dsstore
...
def unzip_file(
    ...
    Examples:
        >>> from rtap.utils.downloads import unzip_file
...
def get_google_drive_file_info(link: str) -> tuple[str, str | None]:
    ...
    Examples:
        >>> from rtap.utils.downloads import get_google_drive_file_info
...
def safe_download(
    ...
    if "://" not in url and Path(url).is_file():  # local file path ('://' check required in Windows Python<3.10)
        ...
        if not f.is_file():  # URL and file do not exist
            uri = (url if gdrive else clean_url(url)).replace(ASSETS_URL, "")  # clean
...
def get_github_assets(
    ...
    Examples:
        >>> tag, assets = get_github_assets(repo="rtap/assets", version="latest")
    """
    import requests  # scoped as slow import
...
def attempt_download_asset(
    ...
    **kwargs,
    ...
    Examples:
        >>> file_path = attempt_download_asset("yolo26n.pt", repo="rtap/assets", release="latest")
    """
    from rtap.utils import SETTINGS  # scoped for circular import

    repo = repo if repo is not None else GITHUB_ASSETS_REPO

    # YOLOv3/5u updates
    file = str(file)
    file = checks.check_rtapv5u_filename(file)
    ...
    if file.exists():
        ...
        if str(file).startswith(("http:/", "https:/")):  # download
            ...
            raise FileNotFoundError(
                f"Weight '{name}' not found locally. Place it in '{SETTINGS['weights_dir']}' "
                "or set RTAP_WEIGHTS_REPO=owner/repo to enable remote downloads."
            )
        else:
