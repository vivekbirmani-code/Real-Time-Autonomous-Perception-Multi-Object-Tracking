...
from rtap.utils import NOT_MACOS14, TORCH_VERSION
from rtap.utils.checks import check_version
from rtap.utils.torch_utils import get_torch_device_backend


class Profile(contextlib.ContextDecorator):
    """RTAP Profile class for timing code execution.
...
def masks2segments(masks: np.ndarray | torch.Tensor, strategy: str = "all") -> list[np.ndarray]:
    ...
    """
    from rtap.data.converter import merge_multi_segment
...
def linear_sum_assignment(cost_matrix):
    ...
    algorithm (Crouse 2016). This keeps SciPy out of RTAP' required dependencies while preserving its speed when
    present. SciPy is imported lazily so it never slows `import rtap`. For a rectangular matrix only min(rows,
