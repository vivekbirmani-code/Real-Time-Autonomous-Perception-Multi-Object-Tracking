...
from rtap.utils import IS_COLAB, IS_KAGGLE, LOGGER, TryExcept, ops, plt_settings, threaded
from rtap.utils.checks import check_font, check_version, is_ascii
from rtap.utils.files import increment_path
from rtap.utils.torch_utils import TORCH_1_10
...
class Colors:
    """RTAP color palette for visualization and plotting.

    This class provides methods to work with the RTAP color palette, including converting hex color codes to RGB
    ...
    !!! note "RTAP Brand Colors"

        For RTAP brand colors see [
        Please use the official RTAP colors for all marketing materials.
    ...
    Examples:
        >>> from rtap.utils.plotting import Colors
        >>> colors = Colors()
    ...
    def __init__(self):
        """Initialize the RTAP color palette from a fixed list of hex color codes."""
        hexs = (
            "042AFF",
            "0BDBEB",
            "F3F3F3",
...
class Annotator:
    """RTAP Annotator for train/val mosaics and JPGs and predictions annotations.
...
def save_one_box(
    ...
    Examples:
        >>> from rtap.utils.plotting import save_one_box
        >>> xyxy = [50, 50, 150, 150]
    ...
    if save:
        ...
        # cv2.imwrite(f, crop)  # save BGR, https://github.com/rtap/yolov5/issues/7007 chroma subsampling issue
...
def plot_results(file: str = "path/to/results.csv", dir: str = "", on_plot: Callable | None = None):
    ...
    Examples:
        >>> from rtap.utils.plotting import plot_results
...
def plot_multitrain_results(scores: dict, key: str = "fitness", save_dir=Path()):
    ...
    Examples:
        >>> from rtap.utils.plotting import plot_multitrain_results
