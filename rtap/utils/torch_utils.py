...
from rtap.utils.cpu import CPUInfo
...
def select_device(device="", newline=False, verbose=True):
    ...
    s = f"RTAP {__version__} 🚀 Python-{PYTHON_VERSION} torch-{TORCH_VERSION} "
...
def model_info_for_loggers(trainer):
    ...
    Args:
        trainer (rtap.engine.trainer.BaseTrainer): The trainer object containing model and validation data.
...
def get_flops(model, imgsz=640):
    ...
    try:
        from rtap.nn.modules.block import AAttn, Attention  # imported here: block.py imports this module
        from rtap.nn.modules.head import RTDETRDecoder
...
def init_seeds(seed=0, deterministic=False):
    ...
    # torch.backends.cudnn.benchmark = True  # AutoBatch problem https://github.com/rtap/yolov5/issues/9287
    if deterministic:
        if TORCH_2_0:
...
def strip_optimizer(f: str | Path = "best.pt", s: str = "", updates: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    Examples:
        >>> from pathlib import Path
        >>> from rtap.utils.torch_utils import strip_optimizer
    ...
    try:
        ...
        LOGGER.warning(f"Skipping {f}, not a valid RTAP model: {e}")
        return {}
...
def profile_ops(input, ops, n=10, device=None, max_num_obj=0):
    """RTAP speed, memory and FLOPs profiler.
    ...
    Examples:
        >>> from rtap.utils.torch_utils import profile_ops
